...
trj_device <- trajectory() %>% 
  ...
  #trap(function() paste0(get_attribute(env, "device"),"_", get_attribute(env, "retry"), "_sai_vlr_reject"), handler = trajectory() %>% set_attribute("sai_vlr_reject", 1) %>% join(trj_reject)) %>%
  #trap(function() paste0(get_attribute(env, "device"),"_", get_attribute(env, "retry"), "_sai_sgsn_reject"), handler = trajectory() %>% set_attribute("sai_sgsn_reject", 1) %>% join(trj_reject)) %>%
  #trap(function() paste0(get_attribute(env, "device"),"_", get_attribute(env, "retry"), "_ul_reject"), handler = trajectory() %>% set_attribute("sai_ul_reject", 1) %>% join(trj_reject)) %>%
  #trap(function() paste0(get_attribute(env, "device"),"_", get_attribute(env, "retry"), "_ul_gprs_reject"), handler = trajectory() %>% set_attribute("sai_ul_gprs_reject", 1) %>% join(trj_reject)) %>%
  #log_("Start SAI!") %>% 
  timeout(function() {
    dev <<- c(dev, get_attribute(env, "device"))
