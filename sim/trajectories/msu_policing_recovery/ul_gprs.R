...
trj_ul_gprs <- trajectory() %>% 
  ...
  leave(prob = function() { ifelse(get_global(env, "ul_gprs_current") >= config$policing$ul_gprs, 1, 0)},
        out = trajectory() %>% 
          set_attribute("rejected", 1) %>% 
          set_global("ul_gprs_rejected", 1, "+") %>% 
          send(function() paste0(get_attribute(env, "device"),"_", get_attribute(env, "retry"), "_ul_gprs_reject"))) %>%
