trj_sai_unknown_hlr_hit <- trajectory() %>% 
  set_attribute("hlr_hit", 1) %>%
  join(trj_hlr_hit) %>% 
  join(trj_cmap_out)

trj_sai_unknown_spc_hit <- trajectory() %>% 
  set_attribute("spc_hit", 1) %>%
  join(trj_spc_hit) %>% 
  join(trj_cmap_out)

trj_sai_unknown_spc_miss <- trajectory() %>% 
...
trj_sai_unknown_hlr_miss <- trajectory() %>% 
  ...
  branch(function() is_cached_spc(get_attribute(env, "device"), simmer::now(env)),
         trj_sai_unknown_spc_hit,
         trj_sai_unknown_spc_miss,
         continue = c(T, T))

trj_sai_unknown <- trajectory() %>% 
  set_global("saiCurrent", 1, "+") %>% 
  join(trj_set_attributes) %>% 
  join(trj_cmap_in) %>% 
  branch(function() is_cached_hlr(get_attribute(env, "device"), simmer::now(env)),
         trj_sai_unknown_hlr_hit,
         trj_sai_unknown_hlr_miss,
         continue = c(T, T)) %>% 
