...
trj_sai_vlr <- trajectory() %>% 
  ...
  leave(prob = function() { ifelse(get_global(env, "sai_vlr_current") >= config$policing$sai_vlr, 1, 0)},
        out = trajectory() %>% 
          set_attribute("rejected", 1) %>% 
          set_global("sai_vlr_rejected", 1, "+") %>% 
          send(function() paste0(get_attribute(env, "device"),"_", get_attribute(env, "retry"), "_sai_vlr_reject"))) %>% 
...
trj_sai_sgsn <- trajectory() %>% 
  join(trj_set_attributes) %>% 
  leave(prob = function() { ifelse(get_global(env, "sai_sgsn_current") >= config$policing$sai_sgsn, 1, 0)},
        out = trajectory() %>% 
          set_attribute("rejected", 1) %>% 
          set_global("sai_sgsn_rejected", 1, "+") %>% 
          send(function() paste0(get_attribute(env, "device"),"_", get_attribute(env, "retry"), "_sai_sgsn_reject"))) %>% 
  set_attribute("rejected", 0) %>% 
