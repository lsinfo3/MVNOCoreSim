...
trj_sai_vlr <- trajectory() %>% 
  ...
  leave(prob = function() { ifelse(get_global(env, "devices_current") >= config$policing$n_dev, 1, 0)},
        out = trajectory() %>% 
          set_attribute("rejected", 1) %>% 
          set_global("device_rejected", 1, "+") %>% 
          send(function() paste0(get_attribute(env, "device"),"_", get_attribute(env, "retry"), "_device_reject"))) %>% 
